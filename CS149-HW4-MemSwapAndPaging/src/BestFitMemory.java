...
public class BestFitMemory {
	 ...
	 */
	// Gets best available spot in mainMemory
	public int getFreeIndex(LinkedList<Process> mainMemory, int memSize) {
		int bestFitIndex = -1; // index of process
		int currentSmallest = 0; // size in MB
		int position = 0; // current position (index) in linkedlist

		for (Process p : mainMemory) {
			// Check if free space chunk is large enough to hold the new process
			if (p.processID == '.' && memSize <= p.memorySize) {
				// If currentSmallest is 0 or if p.memSize is smaller than the
				// current smallest, set the memory's current process' memSize
				// to be the current smallest
				if (currentSmallest == 0 || p.memorySize < currentSmallest) {
					currentSmallest = p.memorySize;
					bestFitIndex = position;
				}
			}
			position++;
		}
	...
	public int runFirstFit(Queue<Process> processQ) {
		...
		while (!processQueue.isEmpty() || !waitingQueue.isEmpty()) {
			...
			while (processQueue.peek() != null
					...
					&& getFreeIndex(mainMemory, processQueue.peek().memorySize) != -1) {
