...
int HSLEEP[2] = { 1, 2 };		//sleeping time for each seller
int MSLEEP[3] = { 2, 3, 4 };
...
Seller::Seller() :
		pricelevel(0), id(0), n(0), midjump(0) {
}
...
bool Seller::checkAvailability()//check availability, if there is seat, then return true. Otherwise return false
{
	...
	if (this->pricelevel == 0)	//since all seats was checked
		...
		for (int i = 0; i < 100; i++) {
			...
			if (seat->getSold() == false) {
				...
				ss2 << serve->getName() << " buy ticket successfully and leave "
						<< this->name << " queue! :D";
				string = ss2.str();
				print(string);
				return true;
		...
		for (int j = 0; j < queue.size(); j++)//At this point, all seats was sold, then return false
			...
			ss3 << serve->getName() << " cannot buy ticket and leave "
					<< this->name << " queue! :(";
			string = ss3.str();
			print(string);
			TURNAWAY++;
		...
		if (midjump == 40) {
			for (int i = 40; i < 60; i++)			//row 5 and 6
				...
				if (seat->getSold() == false) {
					...
					std::stringstream ss2;
					ss2 << serve->getName()
							<< " buy ticket successfully and leave "
							<< this->name << " queue! :D";
					string = ss2.str();
					print(string);
					return true;
	...
	if (this->pricelevel == 2) {
		...
		std::stringstream ss3;
		ss3 << serve->getName() << " cannot buy ticket and leave " << this->name
				<< " queue! :D";
		string = ss3.str();
		print(string);
		TURNAWAY++;
	...
	if (!timesUp) {
		...
		for (int i = 0; i < queue.size(); i++) {
			...
			if (customer->getWaitTime() > 10)			//customer impatient
					{
				std::stringstream ss;
				ss << customer->getName() << "have wait"
						<< customer->getWaitTime() << " leave"
						<< this->getName() << " Queue  because of impatient";
				std::string string = ss.str();
				print(string);
				TURNAWAY++;
				IMPATIENT++;
...
std::string Seat::getName() {
	...
	if (seller == NULL && customer == NULL) {
		...
		ss << (seller->getName()).c_str() << std::setfill('0') << std::setw(3)
				<< customer->getID();
	}
	string = ss.str();
	return string;
...
void customerArrives(Customer* customer) {
	...
	if (timesUp == 1) {
		string = customer->getName()
				+ "arrive and leave because all tickets are sold out!";
		print(string);
		TURNAWAY++;
		return;
	}
	...
	if (success) {
		string = customer->getName() + " arrives " + seller->getName()
				+ " and waits.";
		print(string);
	} else {
		stringstream ss;
...
int main() {
	...
	int N;
	cout
			<< "Please enter the total number of customer per ticket seller(queue size)";
	cin >> N;
	...
	for (int i = 0; i < (queue->size()); i++) {
		string = "From " + highseller->getName() + ", "
				+ (queue->at(i))->getName() + " and leave.";
		print(string);
		TURNAWAY++;
	}
	...
	for (int i = 0; i < 3; i++) {
		...
		for (int j = 0; j < (queue->size()); j++) {
			string = "From " + mediumseller->getName() + ", "
					+ (queue->at(j))->getName() + " and leave.";
			print(string);
			TURNAWAY++;
		}
	...
	for (int i = 0; i < 6; i++) {
		...
		for (int j = 0; j < (queue->size()); j++) {
			string = "From " + lowerseller->getName() + ", "
					+ (queue->at(j))->getName() + " and leave.";
			print(string);
			TURNAWAY++;
		}
